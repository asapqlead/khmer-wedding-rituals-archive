...
export default function GlossaryModal({ onClose, langMode, onScrollTopChange }) {
  ...
  const cleanText = (str) => (str || "").replace(/[\u200B]/g, "").replace(/\s+/g, " ").trim().toLowerCase();
  ...
  const filtered = glossaryTerms.filter((t) => {
    ...
    if (!q) return true;
    return (
      cleanText(t.khmer).includes(q) ||
      cleanText(t.romanized).includes(q) ||
      cleanText(t.category).includes(q) ||
      cleanText(t.definitionEn).includes(q) ||
      cleanText(t.definitionKhmer).includes(q)
    );
  });
  ...
  return (
    ...
    >
      ...
      <div className={`page-view ${isVisible && !isClosing ? "is-visible" : "is-exiting"}`} style={{ maxWidth: 840, margin: "0 auto" }}>
        {/* Header Section: Eyebrow + Title & Search */}
        <div style={{ marginBottom: 28 }}>
          ...
          >
            <WheelText
              ...
              style={{ fontSize: isMobile ? "clamp(20px, 5.5vw, 28px)" : "clamp(24px, 3.5vw, 32px)", color: "#FFFFFF" }}
              as="h2"
            />
            <GlossarySearchInput
              value={searchQuery}
        ...
        {filtered.length > 0 ? (
          ...
          <div style={{ textAlign: "center", padding: isMobile ? "48px 16px 32px" : "72px 20px 48px" }}>
            ...
            <p className={`lang-text ${isKhmer ? "khmer-serif" : ""}`} style={{ fontSize: isMobile ? 16 : 18, color: "#FFFFFF", marginBottom: 8, fontWeight: 400 }}>
              {isKhmer ? "រកមិនឃើញពាក្យដែលត្រូវគ្នា" : "No matching terminology"}
            </p>
            <p className={`lang-text ${isKhmer ? "khmer-sans" : ""}`} style={{ fontSize: isMobile ? 12 : 13, color: "var(--text-muted)", marginBottom: 24, maxWidth: 380, margin: "0 auto 24px", lineHeight: 1.6 }}>
              {isKhmer
